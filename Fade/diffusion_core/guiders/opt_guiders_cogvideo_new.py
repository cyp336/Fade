...
class BaseGuider:
    def __init__(self):
        self.clear_outputs()
        self.store_attnmap_and_feature = False
    ...
    def model_patch(guider_self, model, self_attn_layers_num=None):
        def new_forward_info(self):
            def patched_forward(
                ...
                key = self.to_k(hidden_states)
                value = self.to_v(hidden_states)
                if guider_self.store_attnmap_and_feature == True:
                    #my code
                    query_h2b = self.head_to_batch_dim(query)
                    key_h2b = self.head_to_batch_dim(key)
                    value_h2b = self.head_to_batch_dim(value)
        ...
        # #my code
        # def register_attn(module, layers_num, cur_layers_num=0, parent_name=''):
        #     # 构建当前模块的完整名称
        #     module_name = f'{parent_name}.{module.__class__.__name__}' if parent_name else module.__class__.__name__
            
        #     if 'Attention' in module.__class__.__name__:
        #         if 2 * layers_num[0] <= cur_layers_num < 2 * layers_num[1]:
        #             print(module_name)  # 打印完整路径名称
        #             module.forward = new_forward_info(module)
        #         return cur_layers_num + 1
        #     elif hasattr(module, 'children'):
        #         for module_ in module.children():
        #             # 递归调用时传递当前模块的完整名称作为父级名称
        #             cur_layers_num = register_attn(module_, layers_num, cur_layers_num, module_name)
        #         return cur_layers_num
