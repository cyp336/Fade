...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        batch, num_frames, channels, height, width = image_embeds.shape #[2,3,16,60,90]
        image_embeds = image_embeds.reshape(-1, channels, height, width) #[6,16,60,90]
        image_embeds = self.proj(image_embeds) #[6,1920,30,45]
        image_embeds = image_embeds.view(batch, num_frames, *image_embeds.shape[1:]) #[2,3,1920,30,45]
        image_embeds = image_embeds.flatten(3).transpose(2, 3)  # [batch, num_frames, height x width, channels] [2,3,1350,1920]
        image_embeds = image_embeds.flatten(1, 2)  # [batch, num_frames x height x width, channels] [2,4050,1920]
