...
class GuidanceEditing:
    ...
    def train(
        ...
        if self.config.start_latent == 'inversion':
            ...
            self.inv_latents = torch.load("../inversion_output/all_latent_ddim/dog_walking.pt",map_location='cuda:0')
            ...
            all_mask = torch.load("../mask/dog_walking.pt") #[9,480,720]
            ...
            for i in range(mask.shape[0]):
                ...
                plt.savefig(f"../mask_visualize/dog_walking_{i}.png")
    ...
    def edit(self):
        ...
        return latent2video(self.model, latents, f"../edit_output/dog_walking/5b_4attn_wholemask_{self.config.guiders[1].kwargs.self_attn_gs}_{self.config.guiders[1].kwargs.app_gs}_30_deticmask_3kerneldilate_45local_fft(20,30)_rescale3.mp4",
                            4,(640,640))
